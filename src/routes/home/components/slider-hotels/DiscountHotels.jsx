...
const DiscountHotels = ({ DiscountHotelsData }) => {
  ...
  const [visibleHotels, setVisibleHotels] = useState(5);
    ...
    <div className="px-2 md:px-0 container mx-auto mt-10">
      ...
      <div className="relative">
        {currentIndex > 0 && (
          <button
            onClick={prevSlide}
            className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-1/4 p-2 bg-slate-50 text-slate-700 rounded-full hover:bg-opacity-75 focus:outline-none z-10 w-10 h-10 flex items-center justify-center"
          >
        ...
        <div className="overflow-hidden">
          ...
          >
            {hotelsWithFavoriteStatus.map((hotel) => (
              <div
                key={hotel.Id}
                className="w-1/2 shrink-0 md:w-1/5  transition-transform duration-300 transform hover:scale-105 relative p-2"
              >
                <div className="bg-white h-full shadow-md rounded-lg overflow-hidden">
                  <img
                    src={hotel.images}
                    alt={hotel.name}
        ...
        {currentIndex + visibleHotels < hotelsWithFavoriteStatus.length && (
          <button
            onClick={nextSlide}
            className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-1/4 p-2 bg-slate-50 text-slate-700 rounded-full hover:bg-opacity-75 focus:outline-none z-10 w-10 h-10 flex items-center justify-center"
          >
